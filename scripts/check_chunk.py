...
img_index = int(input('Please choose the index of the image to analyze: '))

# Outputs the values (manually look if they're not absurd)
print("\n--- Data Value Checks ---")
print(f"Run/Event ID:  {meta_data[img_index][0]:.0f}, {meta_data[img_index][1]:.0f}")
print(f"Total Energy:  {meta_data[img_index][2]:.2f} MeV")
print(f"Max pixel value in chunk:  {images[img_index].max():.4f}")
print(f"Min pixel value in chunk:  {images[img_index].min():.4f}")

# Visual proof
print("\n--- Generating Visual Proof ---")
print("Close the plot window to exit the script.")

# Grab the chosen event's image
image = images[img_index]

# Plot it
fig, ax = plt.subplots(figsize=(8, 8))
# We use origin='lower' so the CAL stays at the bottom and TKR is at the top
mesh = ax.imshow(image, cmap='plasma', origin='lower', vmin=0, vmax=1)
fig.colorbar(mesh, ax=ax, label='Normalized Log10(Energy)', shrink=0.8)

ax.set_title(f'Unified LAT Tensor (Run {meta_data[img_index][0]:.0f}, Event {meta_data[img_index][1]:.0f})')
